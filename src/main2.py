...
def is_in_cartesian_prod(x, y, r):
    cartesian_product = [i+j for i in x.split(',') for j in y.split(',')]
    #print(cartesian_product)
    return r in cartesian_product
...
def read_grammar(filename="./grammar.txt"):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            # Get rid of ''
            value = value.replace("'", "")
            # If the value are only two characters, get rid of the space
            #if len(value) == 3:
            value = value.replace(" ", "")
            ...
            for k in range(i, j): # partition
                for lhs in G.keys():
                    for rhs in G[lhs]:
                        ...
                        if len(rhs) > 2: #rules of form A -> B F@$@F
                            if is_in_cartesian_prod(DP_table[i][k], DP_table[k+1][j], rhs):
                                if not lhs in DP_table[i][j]:
                                    DP_table[i][j] = lhs if not DP_table[i][j] else DP_table[i][j] + ',' + lhs
